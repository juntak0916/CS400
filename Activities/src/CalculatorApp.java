...
public class CalculatorApp {
        protected static class SubtractionOperation implements MathOperation{
                @Override
                public double compute(double a, double b) {
                        return a-b;
                }
    ...
    public static void main(String[] args) {
        ...
        for(int b = 1; b < 6; b++) // second operand (b) goes from 1 to 5
            for(int a = b + 1; a < 6; a++) { // first operand goes from b+1 to 5
                System.out.print("     "+a+","+b+":"); // print out operands fir                                                                                                                                                             st
                for(MathOperation op: ops)
                    if(op != null) // then print out result of operation for tho                                                                                                                                                             se available
                        System.out.printf( "%5.1f", op.compute(a,b) );
                    else // and print out a dash otherwise
                        System.out.print("    -");
                System.out.println(); // ensure that the next operands are print                                                                                                                                                             ed to the next line
            }
    }
}
